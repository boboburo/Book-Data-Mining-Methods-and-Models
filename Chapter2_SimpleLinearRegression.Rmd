...
qqline( x,col="red" )   #Line where the it should lie

#Normal test
sw=shapiro.test(x)
ad=nortest::ad.test(x)

text(2,0, cex=.8, pos=4, paste(" mean =", round(mean(x), digits=2),
															 "\n obs = ", length(x) ,
															 "\n standard dev. =", round(sd(x,na.rm=TRUE),digits=2),
															 "\n skewness =", round(e1071::skewness(x), digits=2),
...
op <- par(fig=c(.5,.98,.02,.6), new=TRUE)
text(1,9,"Hello World Default")
box()
par(op)
