...
Construct a visual plot to examine the Standardized reisdual an the Fitted Value.
...
p1<-ggplot(d,aes(x=yh,y=r),label=names)+
	geom_point(size=2)+
	labs(x="Fitted Values",y="Residuals",title="Fitted values vs. Residuals")+
	geom_text(aes(label=ifelse(abs(rs)>1.5,as.character(names),'')),hjust=0.5,vjust=-1,color="red")+
...
p2<-ggplot(cereals,aes(x=sodium))+geom_histogram()


library(gridExtra)
grid.arrange(p2,p1,ncol=1,nrow=2,heights=c(1,4))
