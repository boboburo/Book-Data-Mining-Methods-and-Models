...
c+geom_point()+geom_text(aes(label=Student),hjust=0, vjust=0)+geom_smooth(method="lm", size=0.5, se=T)
```
...
```{r kable1,echo=FALSE}
library(knitr)
kable(summary(fit)$coef, digits=4,format = "markdown")
```

```{r pander1,echo=FALSE}
library(pander)
panderOptions("digits", 3)
pander(anova(fit),caption="ANOVA Table of lm fit")
```
...
pander(datapredict[order(-abs(d1.f.test)),],caption="Sorted by abs(d1.f.test)")
```
